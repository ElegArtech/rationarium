...
export const formaterMoisCourt = (valeur: Date): string => {
  const mois = new Intl.DateTimeFormat(localeDate(), { month: "short", timeZone: "UTC" }).format(valeur);
...
export const formaterMois = (valeur: string | Date | null | undefined): string => {
  const d = instant(valeur);
  if (!d) return "—";
  const rendu = new Intl.DateTimeFormat(localeDate(), {
    month: "long",
    year: "numeric",
    timeZone: "UTC",
  }).format(d);
...
export const formaterMoisSeul = (valeur: string | Date | null | undefined): string => {
  const d = instant(valeur);
  if (!d) return "—";
  return new Intl.DateTimeFormat(localeDate(), { month: "long", timeZone: "UTC" }).format(d);
};
...
export const formaterNombre = (n: number, decimales = 0): string =>
  new Intl.NumberFormat(localeNombre(), {
...
export const formaterJourMoisCourt = (valeur: Date): string =>
  new Intl.DateTimeFormat(localeDate(), {
    day: "2-digit",
    month: "2-digit",
    timeZone: "UTC",
  }).format(valeur);
