...
- **RG-PRJ-12** — Retirer un membre, un tiers ou un client d'un projet défait **le rattachement, les affectations aux tâches de ce projet, et les rôles RACI portés sur ces tâches**. L'entité elle-même — compte, tiers, client — n'est jamais supprimée, et le **temps déclaré est conservé** : `RG-PRJ-08` en calcule le budget consommé. Une tâche dont le seul assigné est retiré demeure, sans assigné. La confirmation annonce le nombre d'affectations concernées **avant** le geste.
  ...
  *Écrit le 2026-09-06, à l'usage.* Le cadrage était muet sur les effets du retrait ; l'implémentation avait tranché seule et affichait « le temps déclaré et les tâches assignées sont conservés ». La seconde moitié contredisait `RG-SCOPE-02` — un projet est visible à ses **membres** : l'affectation conservée laissait quelqu'un porteur d'une tâche d'un projet qu'il ne peut plus ouvrir, et `RG-TSK-03` refuse par ailleurs d'y créer une tâche sans être membre. Le **rôle RACI** relève du même raisonnement, et il a été ajouté à la règle le même jour : « garant » d'une tâche d'un projet qu'on ne peut plus ouvrir est une responsabilité que personne ne peut plus exercer. La **participation aux événements** du projet, elle, n'est PAS emportée — une réunion passée où quelqu'un était présent est une trace, au même titre que le temps déclaré, et non un attachement à défaire. Pour un tiers, `RG-TRS-04` dit la même chose de l'assignation. Le détachement d'un tiers et d'un client n'existait par ailleurs **nulle part** : un tiers rattaché par erreur ne se retirait qu'en supprimant le tiers.
